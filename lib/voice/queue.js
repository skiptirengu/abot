...
function pushQueue(chanId, guildId, playCallback) {
  ...
  musicQueue.onNext(() => {
    log.error(`[${guildId}] Received next. Skipping...`);
    next(guildId);
  });
  musicQueue.onError(err => {
    log.error(`[${guildId}] Music queue error`, err);
    next(guildId);
  });
...
function play(chanId, guildId, input, times = 1, inputArgs = [], outputArgs = []) {
  pushQueue(chanId, guildId, (vcInfo, callback) => {
    ...
    encoder.play();
    log.info(`[${guildId}] Spawned new ffmpeg encoder`);
    encoder.once('end', () => {
      log.info(`[${guildId}] ffmpeg encoder exited`);
      callback();
    });
  }).then(() => {
    // decrease counter
...
function cleanupVoice(guildId) {
  ...
  if (voice !== undefined) {
    try {
      ...
      process.nextTick(() => {
        log.info(`[${guildId}] Cleaning up voice connection`);
